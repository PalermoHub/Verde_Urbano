...
document.addEventListener('DOMContentLoaded', async function() {
    window.VU_DEBUG && console.log('🚀 Inizializzazione...');
    ...
    window.VU_DEBUG && console.log('📥 Step 1: Caricamento CSV...');
    ...
    window.VU_DEBUG && console.log('✅ Step 1 completato - rawGeoJson:', rawGeoJson ? `${rawGeoJson.features.length} features` : 'NULL');

    // Carica i dati foglie stagionali (opzionale)
    window.VU_DEBUG && console.log('📥 Step 2: Caricamento foglie stagionali...');
    ...
    window.VU_DEBUG && console.log('✅ Step 2 completato');

    window.VU_DEBUG && console.log('📥 Step 3: Processamento dati...');
    ...
    loadData();
    window.VU_DEBUG && console.log('✅ Step 3 completato - allTrees:', allTrees.length);
    ...
    window.VU_DEBUG && console.log('📥 Step 4: Popolamento filtri...');
    if (window.updateLoaderStatus) window.updateLoaderStatus('<i class="fas fa-filter"></i> Configurazione filtri...');
    populateFilterSelects();
    window.VU_DEBUG && console.log('✅ Step 4 completato');

    window.VU_DEBUG && console.log('📥 Step 5: Inizializzazione grafici...');
    if (window.updateLoaderStatus) window.updateLoaderStatus('<i class="fas fa-chart-bar"></i> Preparazione grafici...');
    initCharts();
    window.VU_DEBUG && console.log('✅ Step 5 completato');

    window.VU_DEBUG && console.log('📥 Step 6: Applicazione filtri...');
    ...
    applyFilters();
    window.VU_DEBUG && console.log('✅ Step 6 completato - filteredTrees:', filteredTrees.length);

    window.VU_DEBUG && console.log('✅ Inizializzazione completata');
